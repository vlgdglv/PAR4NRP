...
LLAMAGEN_CKPT=${LLAMAGEN_CKPT:-/jizhicfs/pkuhetu/bht/model_home/LlamaGen/c2i_L_256.pt}
RESULTS=${RESULTS:-/dockerdata/bht/LlamaGenNRP/rowar_l_ur_glat_256_fast}
NPROC=${NPROC:-8}
...
torchrun --nproc_per_node=${NPROC} --master_port=${MASTER_PORT:-26312} \
   -m autoregressive.train.train_c2i_rowar \
    ...
    --num-workers 8 \
    --log-every 50 \
    --ckpt-every 2500 --wandb-project LlamaGenNRP
